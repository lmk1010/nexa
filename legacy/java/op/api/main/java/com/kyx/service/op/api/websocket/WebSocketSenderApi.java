package com.kyx.service.op.api.websocket;

import com.kyx.foundation.common.pojo.CommonResult;
import com.kyx.foundation.common.util.json.JsonUtils;
import com.kyx.service.op.api.websocket.dto.WebSocketSendReqDTO;
...
public interface WebSocketSenderApi {
    ...
    default void send(Integer userType, Long userId, String messageType, String messageContent) {
        send(WebSocketSendReqDTO.builder().userType(userType).userId(userId)
                .messageType(messageType).messageContent(messageContent).build()).checkError();
    }
    ...
    default void send(Integer userType, String messageType, String messageContent) {
        send(WebSocketSendReqDTO.builder().userType(userType)
                .messageType(messageType).messageContent(messageContent).build()).checkError();
    }
    ...
    default void send(String sessionId, String messageType, String messageContent) {
        send(WebSocketSendReqDTO.builder().sessionId(sessionId)
                .messageType(messageType).messageContent(messageContent).build()).checkError();
    }
