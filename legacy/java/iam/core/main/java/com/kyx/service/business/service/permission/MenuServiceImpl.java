...
import com.kyx.service.business.controller.admin.permission.vo.menu.MenuSaveVO;
import com.kyx.service.business.dal.dataobject.permission.MenuDO;
import com.kyx.service.business.dal.mysql.permission.MenuMapper;
import com.kyx.service.business.dal.redis.RedisKeyConstants;
import com.kyx.service.business.enums.permission.MenuTypeEnum;
...
import static com.kyx.service.business.dal.dataobject.permission.MenuDO.ID_ROOT;
...
public class MenuServiceImpl implements MenuService {
    ...
    private void deleteMenuRecursively(Long menuId) {
        // 查询所有子菜单
        List<MenuDO> children = menuMapper.selectListByParentId(menuId);
    ...
    void validateMenuComponentName(String componentName, Long id) {
        ...
        // 查询所有相同component_name的菜单，避免selectOne的TooManyResultsException
        List<MenuDO> menus = menuMapper.selectListByComponentName(componentName);
        if (CollUtil.isEmpty(menus)) {
            return;
        }
        
        // 如果 id 为空，说明是新建菜单，只要存在相同component_name就报错
        if (id == null) {
            throw exception(MENU_COMPONENT_NAME_DUPLICATE);
        }
        
        // 更新菜单时，检查是否存在其他菜单使用了相同的component_name
        boolean hasDuplicate = menus.stream()
