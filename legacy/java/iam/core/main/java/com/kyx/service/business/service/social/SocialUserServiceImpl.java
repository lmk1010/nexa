...
import com.kyx.service.business.dal.mysql.social.SocialUserMapper;
...
import static com.kyx.service.business.enums.ErrorCodeConstants.SOCIAL_USER_NOT_FOUND;
...
public class SocialUserServiceImpl implements SocialUserService {
    ...
    public SocialUserRespDTO getSocialUserByUserId(Integer userType, Long userId, Integer socialType) {
        ...
        Assert.notNull(socialUser, "社交用户不能为空");
        return buildSocialUserRespDTO(socialUser, socialUserBind.getUserId());
    }
    ...
    public SocialUserRespDTO getSocialUserByCode(Integer userType, Integer socialType, String code, String state) {
        ...
        SocialUserBindDO socialUserBind = socialUserBindMapper.selectByUserTypeAndSocialUserId(userType,
                socialUser.getId());
        return buildSocialUserRespDTO(socialUser, socialUserBind != null ? socialUserBind.getUserId() : null);
    }

    private SocialUserRespDTO buildSocialUserRespDTO(SocialUserDO socialUser, Long userId) {
        SocialUserRespDTO respDTO = new SocialUserRespDTO(socialUser.getOpenid(), socialUser.getNickname(),
                socialUser.getAvatar(), userId);
        respDTO.setMobile(extractMobile(socialUser));
        return respDTO;
    }

    private String extractMobile(SocialUserDO socialUser) {
        if (socialUser == null || StrUtil.isBlank(socialUser.getRawUserInfo())) {
            return null;
        }
        return JSONUtil.parseObj(socialUser.getRawUserInfo()).getStr("mobile");
    }
