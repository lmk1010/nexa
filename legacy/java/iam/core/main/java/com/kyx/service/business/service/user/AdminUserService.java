...
public interface AdminUserService {
     ...
     * @return 用户对象信息
     */
    AdminUserDO getManageableUser(Long id);
     ...
     * @param id 用户ID
     */
    void validateManageableUser(Long id);

    /**
     * 获得用户信息（忽略租户过滤）
     * 用于多租户场景下，跨租户获取用户基本信息
     *
     * @param id 用户ID
     * @return 用户对象信息
     */
    ...
    /**
     * 根据租户ID列表获取用户列表（支持跨租户查询）
     *
     * @param tenantIds 租户ID列表，逗号分隔。为空则查询当前租户
     * @return 用户列表
     */
    List<AdminUserDO> getUserListByTenants(String tenantIds);

    /**
     * 根据流程模型ID自动判断跨租户模式并获取用户列表
     *
     * @param tenantIds 租户ID列表，逗号分隔。如果提供则优先使用
     * @param modelId 流程模型ID，用于自动判断跨租户模式
     * @return 用户列表
     */
    List<AdminUserDO> getUserListByTenants(String tenantIds, Long modelId);

    /**
     * 根据租户ID列表和功能编码获取用户列表
     *
     * @param tenantIds 租户ID列表，逗号分隔。如提供则优先使用
     * @param modelId 流程模型ID，用于兼容旧调用
     * @param featureCode 功能编码，用于判断是否允许跨租户范围
     * @return 用户列表
     */
    List<AdminUserDO> getUserListByTenants(String tenantIds, Long modelId, String featureCode);
     ...
     */
    void updateUserDeptBatch(Collection<Long> userIds, Long newDeptId);
     ...
     * @return 入职用户信息
     */
    UserOnboardingRespVO createOnboardingUser(UserOnboardingCreateReqVO createReqVO);
