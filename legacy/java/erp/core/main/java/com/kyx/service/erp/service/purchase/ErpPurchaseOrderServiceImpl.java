...
import com.kyx.foundation.common.util.number.MoneyUtils;
...
import com.kyx.service.erp.dal.mysql.purchase.ErpPurchaseOrderItemMapper;
import com.kyx.service.erp.dal.mysql.purchase.ErpPurchaseOrderMapper;
...
import com.kyx.service.erp.service.finance.ErpAccountService;
...
public class ErpPurchaseOrderServiceImpl implements ErpPurchaseOrderService {
     ...
     * 采购订单对应的流程定义 KEY
     */
    public static final String PROCESS_KEY = "purchase-order";
    ...
    private List<ErpPurchaseOrderItemDO> validatePurchaseOrderItems(List<ErpPurchaseOrderSaveReqVO.Item> list) {
        ...
        return convertList(list, o -> BeanUtils.toBean(o, ErpPurchaseOrderItemDO.class, item -> {
            ...
            BigDecimal discountedPrice = item.getProductPrice();
            if (discountPercent.compareTo(BigDecimal.ZERO) > 0) {
                discountedPrice = item.getProductPrice().multiply(BigDecimal.ONE.subtract(discountPercent.divide(BigDecimal.valueOf(100))));
            }
            
            // 计算行总价（折扣后单价 * 数量）
            item.setTotalPrice(MoneyUtils.priceMultiply(discountedPrice, item.getCount()));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long createPurchaseOrderAndSubmit(Long userId, ErpPurchaseOrderSaveReqVO createReqVO) {
        // 1.1 校验订单项的有效性
        List<ErpPurchaseOrderItemDO> purchaseOrderItems = validatePurchaseOrderItems(createReqVO.getItems());
        // 1.2 校验供应商
        supplierService.validateSupplier(createReqVO.getSupplierId());
        ...
        ErpPurchaseOrderDO purchaseOrder = BeanUtils.toBean(createReqVO, ErpPurchaseOrderDO.class, in -> in
                .setNo(no).setStatus(ErpAuditStatus.PROCESS.getStatus()));
        purchaseOrder.setBmpStatus(ErpPurchaseOrderBmpStatusEnum.IN_PROGRESS.getStatus());
        calculateTotalPrice(purchaseOrder, purchaseOrderItems);
        purchaseOrderMapper.insert(purchaseOrder);
        ...
        processInstanceVariables.put("orderId", purchaseOrder.getId());
        processInstanceVariables.put("orderNo", purchaseOrder.getNo());
        processInstanceVariables.put("supplierId", purchaseOrder.getSupplierId());
        processInstanceVariables.put("totalPrice", purchaseOrder.getTotalPrice());
        processInstanceVariables.put("currency", purchaseOrder.getCurrency());
        processInstanceVariables.put("purchaseOrganization", purchaseOrder.getPurchaseOrganization());
        ...
        // 2.4 将工作流的编号，更新到采购订单中
        ErpPurchaseOrderDO updateObj = new ErpPurchaseOrderDO();
        updateObj.setId(purchaseOrder.getId());
    ...
    public void submitPurchaseOrder(Long id, Long userId) {
        // 1.1 校验存在
        ErpPurchaseOrderDO purchaseOrder = validatePurchaseOrderExists(id);
        // 1.2 校验状态：只有待审核状态才能提交
        if (!ErpAuditStatus.PROCESS.getStatus().equals(purchaseOrder.getStatus())) {
            throw exception(PURCHASE_ORDER_SUBMIT_FAIL_STATUS_NOT_PROCESS);
        }

        // 2.1 更新BMP状态为流程中
        ErpPurchaseOrderDO updateObj = new ErpPurchaseOrderDO();
        updateObj.setId(id);
        updateObj.setBmpStatus(ErpPurchaseOrderBmpStatusEnum.IN_PROGRESS.getStatus());
        purchaseOrderMapper.updateById(updateObj);

        // 2.2 发起 BPM 流程
        Map<String, Object> processInstanceVariables = new HashMap<>();
        processInstanceVariables.put("orderId", id);
        processInstanceVariables.put("orderNo", purchaseOrder.getNo());
        processInstanceVariables.put("supplierId", purchaseOrder.getSupplierId());
        processInstanceVariables.put("totalPrice", purchaseOrder.getTotalPrice());
        processInstanceVariables.put("currency", purchaseOrder.getCurrency());
        ...
        // 2.3 将工作流的编号，更新到采购订单中
        updateObj = new ErpPurchaseOrderDO();
        updateObj.setId(id);
        updateObj.setProcessInstanceId(processInstanceId);
        purchaseOrderMapper.updateById(updateObj);
    }
    ...
    @Override
    public void updatePurchaseOrderBmpStatus(Long id, Integer status, Integer bmpStatus) {
