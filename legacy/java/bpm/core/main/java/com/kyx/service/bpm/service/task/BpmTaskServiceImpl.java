...
import com.kyx.service.bpm.controller.admin.task.vo.task.*;
import com.kyx.service.bpm.convert.task.BpmTaskConvert;
...
import com.kyx.service.bpm.enums.definition.*;
import com.kyx.service.bpm.enums.task.BpmCommentTypeEnum;
import com.kyx.service.bpm.enums.task.BpmReasonEnum;
import com.kyx.service.bpm.enums.task.BpmTaskSignTypeEnum;
...
import com.kyx.service.bpm.framework.flowable.core.enums.BpmnVariableConstants;
import com.kyx.service.bpm.framework.flowable.core.util.BpmHttpRequestUtils;
import com.kyx.service.bpm.framework.flowable.core.util.BpmnModelUtils;
import com.kyx.service.bpm.framework.flowable.core.util.FlowableUtils;
import com.kyx.service.bpm.service.definition.BpmFormService;
import com.kyx.service.bpm.service.definition.BpmModelService;
import com.kyx.service.bpm.service.definition.BpmProcessDefinitionService;
import com.kyx.service.bpm.service.message.BpmMessageService;
...
import static com.kyx.service.bpm.framework.flowable.core.enums.BpmnModelConstants.START_USER_NODE_ID;
import static com.kyx.service.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_RETURN_FLAG;
import static com.kyx.service.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_SKIP_START_USER_NODE;
import static com.kyx.service.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    private static final String BPM_TASK_CANDIDATE_NOTICE_SENT = "BPM_TASK_CANDIDATE_NOTICE_SENT";
    ...
    public PageResult<Task> getTaskTodoPage(Long userId, BpmTaskPageReqVO pageVO) {
        ...
        List<BpmProcessDefinitionInfoDO> visibleDefinitions = bpmProcessDefinitionService.getVisibleProcessDefinitionList();
        ...
        tasks = filterList(tasks, task -> visibleProcessDefinitionIds.contains(task.getProcessDefinitionId()));
        return new PageResult<>(tasks, (long) tasks.size());
    }

    @Override
    public BpmTaskRespVO getTodoTask(Long userId, String taskId, String processInstanceId) {
        // 1.1 获取指定的用户待办任务
        Task todoTask = getMyTodoTask(userId, taskId, false);
        // 1.2 获取不到，则获取该流程实例下，第一个用户的待办任务
        if (todoTask == null) {
            todoTask = getMyFirstTodoTask(userId, processInstanceId, false);
        }
        // 1.3 管理员兜底：允许流程详情返回当前运行任务，用于代处理
        if (todoTask == null && isBpmTaskAdmin(userId)) {
            todoTask = getMyTodoTask(userId, taskId, true);
            if (todoTask == null) {
                todoTask = getMyFirstTodoTask(userId, processInstanceId, true);
            }
        }
        if (todoTask == null) {
            return null;
     ...
     * @return 任务
     */
    private Task getMyTodoTask(Long userId, String taskId, boolean taskAdmin) {
     ...
     * @return 任务
     */
    private Task getMyFirstTodoTask(Long userId, String processInstanceId, boolean taskAdmin) {
    ...
    public BpmCopyNoticeResendRespDTO resendRunningTaskDingTalkNotices(Integer limit, Boolean dryRun) {
        int safeLimit = limit == null ? 200 : Math.max(1, Math.min(limit, 500));
        List<Task> tasks = taskService.createTaskQuery().active().orderByTaskCreateTime().desc()
        ...
        for (Task task : tasks) {
            ...
            BpmMessageSendWhenTaskCreatedReqDTO reqDTO = buildTaskAssignedMessage(processInstance, task);
            if (reqDTO == null) {
                continue;
            }
            if (!Boolean.TRUE.equals(dryRun)) {
                FlowableUtils.execute(processInstance.getTenantId(),
                        () -> messageService.sendDingTalkWhenTaskAssigned(reqDTO));
            }
            receiverCount++;
        }
        return new BpmCopyNoticeResendRespDTO()
                .setScannedCount(tasks.size())
                .setRunningCount(tasks.size())
                .setReceiverCount(receiverCount)
                .setTaskScannedCount(tasks.size())
                .setTaskReceiverCount(receiverCount);
    }
    ...
    public Task validateTask(Long userId, String taskId) {
        ...
        if (StrUtil.isNotBlank(task.getAssignee())
                ...
                && !isBpmTaskAdmin(userId)) {
            throw exception(TASK_OPERATE_FAIL_ASSIGN_NOT_SELF);
        }
        BpmProcessDefinitionInfoDO definitionInfo = bpmProcessDefinitionService.getProcessDefinitionInfo(
                task.getProcessDefinitionId());
        if (definitionInfo == null) {
            throw exception(PROCESS_DEFINITION_NOT_EXISTS);
        }
        Long tenantId = TenantContextHolder.getTenantId();
        if (!bpmProcessDefinitionService.hasProcessDefinitionAccess(definitionInfo, tenantId)) {
            throw exception(PROCESS_INSTANCE_ACCESS_DENIED);
        }
        return task;
    }

    private boolean isBpmTaskAdmin(Long userId) {
        if (userId == null) {
            return false;
        }
    ...
    public void approveTask(Long userId, @Valid BpmTaskApproveReqVO reqVO) {
        ...
        processVariables.putIfAbsent(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_HAS_NOTICE_APPROVER, Boolean.FALSE);
        ...
        // 注意：前面 setVariableLocal/addComment 可能已由 Flowable 提前落库；
        // complete 失败时不能静默吞掉，否则会出现“意见已写、任务仍在运行”的半完成状态。
        try {
            taskService.complete(task.getId(), variables, true);
        } catch (Exception ex) {
            log.error("[approveTask][complete 失败][taskId={}, processInstanceId={}, userId={}, reason={}]",
                    task.getId(), task.getProcessInstanceId(), userId, reqVO.getReason(), ex);
            throw ex;
        }
    ...
    private Map<String, Object> validateAndSetNextAssignees(String taskDefinitionKey, Map<String, Object> variables, BpmnModel bpmnModel,
        ...
        for (FlowNode nextFlowNode : nextFlowNodes) {
            ...
            if (ObjUtil.equals(candidateStrategy, BpmTaskCandidateStrategyEnum.APPROVE_USER_SELECT.getStrategy())) {
                ...
                List<Long> assignees = nextAssignees != null ? nextAssignees.get(nextFlowNode.getId()) : null;
                if (CollUtil.isEmpty(assignees)) {
                    Integer assignEmptyHandlerType = BpmnModelUtils.parseAssignEmptyHandlerType(nextFlowNode);
                    if (ObjectUtil.equal(assignEmptyHandlerType, BpmUserTaskAssignEmptyHandlerTypeEnum.APPROVE.getType())) {
                        ...
                        approveUserSelectAssignees.put(nextFlowNode.getId(), Collections.emptyList());
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 foundation-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            /**
             * 优先 afterCommit：在创建任务事务提交后再自动通过，避免半提交状态
             * （状态/意见已写，但 complete 未生效）。
             */
            @Override
            public void afterCommit() {
                autoHandleCreatedTask(task.getId(), approveType, assignEmptyHandlerType);
            }

            /**
             * 兼容：部分情况下 afterCommit 不会被调用，但是 afterCompletion 可以
             * 例如说：第一个 task 就是配置【自动通过】或者【自动拒绝】时
             * 参见 <a href="https://gitee.com/zhijiantianya/foundation-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
            public void afterCompletion(int transactionStatus) {
                ...
                // afterCommit 已处理过的正常提交，不再重复
                if (ObjectUtil.equal(transactionStatus, TransactionSynchronization.STATUS_COMMITTED)) {
                    return;
                }
                // 特殊情况：第一个 task 【自动通过】时 transactionStatus 会为 STATUS_UNKNOWN
     ...
     * 任务创建后的自动通过 / 自动拒绝处理。
     */
    private void autoHandleCreatedTask(String taskId, Integer approveType, Integer assignEmptyHandlerType) {
        ...
        if (ObjectUtil.equal(approveType, BpmUserTaskApproveTypeEnum.USER.getType())) {
            ...
            if (!ObjectUtil.isAllEmpty(currentTask.getAssignee(), currentTask.getOwner())) {
                sendCandidateTaskAssignedNotice(taskId);
                return;
            }
            if (ObjectUtil.equal(assignEmptyHandlerType, BpmUserTaskAssignEmptyHandlerTypeEnum.APPROVE.getType())) {
                try {
                    getSelf().approveTask(null, new BpmTaskApproveReqVO()
                            .setId(currentTask.getId()).setReason(BpmReasonEnum.ASSIGN_EMPTY_APPROVE.getReason()));
                } catch (Exception ex) {
                    log.error("[processTaskCreated][空审批人自动通过失败][taskId={}, processInstanceId={}, taskKey={}]",
                            currentTask.getId(), currentTask.getProcessInstanceId(),
                            currentTask.getTaskDefinitionKey(), ex);
                }
                ...
                try {
                    getSelf().rejectTask(null, new BpmTaskRejectReqVO()
                            .setId(currentTask.getId()).setReason(BpmReasonEnum.ASSIGN_EMPTY_REJECT.getReason()));
                } catch (Exception ex) {
                    log.error("[processTaskCreated][空审批人自动拒绝失败][taskId={}, processInstanceId={}, taskKey={}]",
                            currentTask.getId(), currentTask.getProcessInstanceId(),
                            currentTask.getTaskDefinitionKey(), ex);
                }
            }
            ...
            if (ObjectUtil.equal(approveType, BpmUserTaskApproveTypeEnum.AUTO_APPROVE.getType())) {
                try {
                    getSelf().approveTask(null, new BpmTaskApproveReqVO()
                            .setId(currentTask.getId()).setReason(BpmReasonEnum.APPROVE_TYPE_AUTO_APPROVE.getReason()));
                } catch (Exception ex) {
                    log.error("[processTaskCreated][自动审核通过失败][taskId={}, processInstanceId={}, taskKey={}]",
                            currentTask.getId(), currentTask.getProcessInstanceId(),
                            currentTask.getTaskDefinitionKey(), ex);
                }
                ...
                try {
                    getSelf().rejectTask(null, new BpmTaskRejectReqVO()
                            .setId(currentTask.getId()).setReason(BpmReasonEnum.APPROVE_TYPE_AUTO_REJECT.getReason()));
                } catch (Exception ex) {
                    log.error("[processTaskCreated][自动审核拒绝失败][taskId={}, processInstanceId={}, taskKey={}]",
                            currentTask.getId(), currentTask.getProcessInstanceId(),
                            currentTask.getTaskDefinitionKey(), ex);
                }
            }
    ...
    public void processTaskAssigned(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(int transactionStatus) {
                ...
                // 自动去重，通过自动审批的方式 TODO @ 驳回的情况得考虑一下；@lesan：驳回后，又自动审批么？
    ...
    private void sendCandidateTaskAssignedNotice(String taskId) {
        Task currentTask = getTask(taskId);
        if (currentTask == null || StrUtil.isNotEmpty(currentTask.getAssignee())) {
            return;
        }
        Object sentFlag = taskService.getVariableLocal(taskId, BPM_TASK_CANDIDATE_NOTICE_SENT);
        if (Boolean.TRUE.equals(sentFlag)) {
            return;
        }
        Set<Long> candidateUserIds = getCandidateUserIds(currentTask.getId());
        ...
        String startUserNickname = resolveStartUserNickname(processInstance);
        ...
        for (Long candidateUserId : candidateUserIds) {
            BpmMessageSendWhenTaskCreatedReqDTO reqDTO = new BpmMessageSendWhenTaskCreatedReqDTO()
                    ...
                    .setStartUserNickname(startUserNickname)
                    .setTaskId(currentTask.getId())
                    .setTaskName(currentTask.getName())
                    .setAssigneeUserId(candidateUserId);
            messageService.sendMessageWhenTaskAssigned(reqDTO);
        }
        taskService.setVariableLocal(taskId, BPM_TASK_CANDIDATE_NOTICE_SENT, true);
    }

    private Set<Long> getCandidateUserIds(String taskId) {
        ...
        for (IdentityLink link : identityLinks) {
            ...
            Long userId = NumberUtils.parseLong(link.getUserId());
            if (userId != null) {
                userIds.add(userId);
            }
        }
    ...
    private BpmMessageSendWhenTaskCreatedReqDTO buildTaskAssignedMessage(ProcessInstance processInstance, Task task) {
        Long startUserId = NumberUtils.parseLong(processInstance.getStartUserId());
        if (startUserId == null || StrUtil.isBlank(task.getAssignee())) {
            return null;
        }
        String startUserNickname = resolveStartUserNickname(processInstance);
        return new BpmMessageSendWhenTaskCreatedReqDTO()
                .setProcessInstanceId(processInstance.getProcessInstanceId())
                .setProcessInstanceName(processInstance.getName())
                .setStartUserId(startUserId)
                .setStartUserNickname(startUserNickname)
    ...
    private String resolveStartUserNickname(ProcessInstance processInstance) {
        Long startUserId = NumberUtils.parseLong(processInstance.getStartUserId());
        if (startUserId == null) {
            return "System";
        }
        AdminUserRespDTO startUser = adminUserApi.getUser(startUserId).getCheckedData();
        return startUser == null || StrUtil.isBlank(startUser.getNickname()) ? "System" : startUser.getNickname();
    }
