...
import com.kyx.service.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelNodeVO;
import com.kyx.service.bpm.controller.admin.task.vo.instance.*;
import com.kyx.service.bpm.controller.admin.task.vo.instance.BpmApprovalDetailRespVO.ActivityNodeTask;
import com.kyx.service.bpm.controller.admin.task.vo.task.BpmTaskRespVO;
import com.kyx.service.bpm.convert.task.BpmProcessInstanceConvert;
import com.kyx.service.bpm.dal.dataobject.definition.BpmProcessDefinitionInfoDO;
import com.kyx.service.bpm.dal.redis.BpmProcessIdRedisDAO;
import com.kyx.service.bpm.enums.ErrorCodeConstants;
import com.kyx.service.bpm.enums.definition.BpmModelTypeEnum;
import com.kyx.service.bpm.enums.definition.BpmSimpleModelNodeTypeEnum;
...
import com.kyx.service.bpm.framework.flowable.core.event.BpmProcessInstanceKafkaEventPublisher;
...
import com.kyx.service.bpm.framework.flowable.core.util.SimpleModelUtils;
...
import static com.kyx.service.bpm.controller.admin.task.vo.instance.BpmApprovalDetailRespVO.ActivityNode;
...
import static com.kyx.service.bpm.framework.flowable.core.util.BpmnModelUtils.parseNodeType;
...
public class BpmProcessInstanceServiceImpl implements BpmProcessInstanceService {
    ...
    @Resource
    private BpmProcessInstanceKafkaEventPublisher kafkaEventPublisher;
    ...
    private void validateProcessDefinitionAccess(String processDefinitionId) {
        BpmProcessDefinitionInfoDO definitionInfo = processDefinitionService.getProcessDefinitionInfo(processDefinitionId);
        if (definitionInfo == null) {
            throw exception(PROCESS_DEFINITION_NOT_EXISTS);
        }
        Long tenantId = TenantContextHolder.getTenantId();
        if (!processDefinitionService.hasProcessDefinitionAccess(definitionInfo, tenantId)) {
            throw exception(PROCESS_INSTANCE_ACCESS_DENIED);
        }
    }

    private void validateProcessDefinitionAccessForStart(String processDefinitionId) {
        BpmProcessDefinitionInfoDO definitionInfo = processDefinitionService.getProcessDefinitionInfo(processDefinitionId);
        if (definitionInfo == null) {
            throw exception(PROCESS_DEFINITION_NOT_EXISTS);
        }
    ...
    private void validateProcessInstanceAccess(HistoricProcessInstance instance) {
        if (instance == null) {
            throw exception(PROCESS_INSTANCE_NOT_EXISTS);
        }
        validateProcessDefinitionAccess(instance.getProcessDefinitionId());
    }

    private void validateProcessInstanceAccess(String processInstanceId) {
        HistoricProcessInstance instance = getHistoricProcessInstance(processInstanceId);
        validateProcessInstanceAccess(instance);
    }
    ...
    public BpmApprovalDetailRespVO getApprovalDetail(Long loginUserId, BpmApprovalDetailReqVO reqVO) {
        ...
        if (reqVO.getProcessInstanceId() != null) {
            ...
            validateProcessInstanceAccess(historicProcessInstance);
        ...
        if (historicProcessInstance == null
                && !processDefinitionService.hasProcessDefinitionAccess(processDefinitionInfo, TenantContextHolder.getTenantId())) {
            throw exception(PROCESS_INSTANCE_START_USER_CAN_START);
        }
    ...
    public List<ActivityNode> getNextApprovalNodes(Long loginUserId, BpmApprovalDetailReqVO reqVO) {
        if (reqVO.getProcessInstanceId() != null) {
            validateProcessInstanceAccess(reqVO.getProcessInstanceId());
        } else if (reqVO.getProcessDefinitionId() != null) {
            validateProcessDefinitionAccessForStart(reqVO.getProcessDefinitionId());
        }
        // 1.1 校验任务存在，且是当前用户的
        ...
        List<ActivityNode> nextActivityNodes = convertList(nextFlowNodes, node -> new ActivityNode().setId(node.getId())
                ...
                .setAssignEmptyHandlerType(BpmnModelUtils.parseAssignEmptyHandlerType(node))
    ...
    public PageResult<HistoricProcessInstance> getProcessInstancePage(Long userId,
        ...
        // 过滤出当前租户可见的流程实例
        processInstanceList = filterList(processInstanceList,
            instance -> visibleProcessDefinitionIds.contains(instance.getProcessDefinitionId()));

        return new PageResult<>(processInstanceList, (long) processInstanceList.size());
    }
    ...
    private List<ActivityNode> getEndActivityNodeList(Long startUserId, BpmnModel bpmnModel,
        ...
        List<ActivityNode> approvalNodes = convertList(endTasks, task -> {
            ...
            ActivityNode activityNode = new ActivityNode().setId(task.getTaskDefinitionKey()).setName(task.getName())
                    ...
                    .setAssignEmptyHandlerType(BpmnModelUtils.parseAssignEmptyHandlerType(flowNode))
    ...
    private ActivityNode buildNotRunApproveNodeForSimple(Long startUserId, BpmnModel bpmnModel,
        ...
        ActivityNode activityNode = new ActivityNode().setId(node.getId()).setName(node.getName())
                ...
                .setAssignEmptyHandlerType(getSimpleAssignEmptyHandlerType(node))
    ...
    private Integer getSimpleAssignEmptyHandlerType(BpmSimpleModelNodeVO node) {
        return node == null || node.getAssignEmptyHandler() == null ? null : node.getAssignEmptyHandler().getType();
    }
    ...
    public BpmProcessInstanceBpmnModelViewRespVO getProcessInstanceBpmnModelView(String id) {
        ...
        validateProcessInstanceAccess(processInstance);
        // 1.2 获得流程定义
    ...
    private void validateStartUserSelectAssignees(Long userId, ProcessDefinition definition,
        ...
        // 预测条件表达式可能依赖默认流程变量；缺失时 JUEL 会抛错，这里先补齐避免误报。
        if (variables == null) {
            variables = new HashMap<>();
        }
        variables.putIfAbsent(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_HAS_NOTICE_APPROVER, Boolean.FALSE);
    ...
    public void processProcessInstanceCompleted(ProcessInstance instance) {
        ...
        FlowableUtils.execute(instance.getTenantId(), () -> {
            ...
            // 3. 发送流程实例的状态事件（Spring内存事件）
            processInstanceEventPublisher.sendProcessInstanceResultEvent(
                    BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceStatusEvent(this, instance, status));
            
            // 4. 发送流程实例的状态事件到Kafka（分布式事件）
            kafkaEventPublisher.sendProcessInstanceStatusEvent(
                    BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceStatusKafkaEvent(instance, status));
