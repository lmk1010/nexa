...
**IM** — Go-only service for the nexa enterprise assistant platform.

## Runtime

**Go only.** Java is never a nexa runtime dependency.  
Legacy Java reference (read-only): `legacy/java/im/`.
...
go run ./cmd/nexa-im -config ./configs/config.json
# GET http://127.0.0.1:48087/healthz
```
