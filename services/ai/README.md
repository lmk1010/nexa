...
**AI** — Go-only service for the nexa enterprise assistant platform.

## Runtime

**Go only.** Java is never a nexa runtime dependency.  
Legacy Java reference (read-only): `legacy/java/ai/`.
...
go run ./cmd/nexa-ai -config ./configs/config.json
# GET http://127.0.0.1:48089/healthz
```
